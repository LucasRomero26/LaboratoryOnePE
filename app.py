...
def calculate_ybus(n_nodes, branches):
    ...
    Parámetros:
      - n_nodes: Número de nodos del sistema.
      - branches: Lista de diccionarios que representan las ramas del sistema, cada una con:
          ...
          - 'to': nodo final
          - 'admittance': valor complejo de la admitancia de la rama.
...
def main():
    ...
    with st.form("branch_form"):
        ...
        for i in range(int(n_branches)):
            ...
            y_shunt_imag = col5.number_input("Y/2", step=0.00001, format="%.5f", key=f"yshunt_imag_{i}")
        ...
        if submitted:
            # Validación: los nodos inicial y final no deben ser iguales
            valid = True
    ...
    1. **Cálculo de la matriz Ybus:**  
       La función `calculate_ybus` crea una matriz de ceros y, para cada rama, suma la admitancia a las entradas diagonales y la resta en las posiciones correspondientes a la conexión entre nodos.
    
    2. **Interfaz en Streamlit:**  
       Se solicitan al usuario el número de nodos y ramas, y se utiliza un formulario para ingresar los datos de cada rama (nodo inicial, nodo final, parte real e imaginaria de la admitancia).
    
    3. **Presentación de Resultados:**  
       La matriz resultante se formatea para mostrar cada elemento como un número complejo con dos decimales y se presenta en una tabla para facilitar su lectura.
    """)
